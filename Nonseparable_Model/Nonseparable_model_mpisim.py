...
def train(x, Y, N_opt=None, err_opt=None, N_hmc=1000, learning_rate=1e-1, do_initialization=True, use_separable_res=False, use_empirical_res=False, use_random_res=False, use_combined_res=False, do_MAP=True, do_HMC=True, hyper_pars=None, save_dir=None, folder_name=None, folder_name_separable=None, subfolder_name=None, verbose=False):
    """
        ...
        if err_opt is not None:
            ...
            while gap > err_opt:
                ...
                if i % 100 == 99:
                    ...
                    curr_obj = NegLog
                if verbose and (i % 100 == 99):
                    print("{}/{} iteration with target value {}, loglik: {},s log_prior_tilde_l: {}, log_prior_uL_vecs: {}, log_prior_sigma2_err: {}".format(
                            i+1, N_opt, NegLog, loglik, log_prior_tilde_l, log_prior_uL_vecs, log_prior_sigma2_err))
        fig = plt.figure()
...
if __name__ == "__main__":
    ...
    # x, Y, x_test, Y_test, trend, scale, x_scale = load_syndata(directory="../data/sim/sim_MNTS_{}.pickle".format(rank), test_size = 0.)
    ...
    hyper_pars = {"mu_tilde_l": 0., "alpha_tilde_l": 10., "beta_tilde_l": 1, "mu_L": 0.,
                  "alpha_L": 1., "beta_L": 1, "a": 1e-2, "b": 1e-2}
    ...
    if do_MAP:
        ...
        # try:
        #     NegLog_combined = train(x, Y, err_opt=1, N_hmc=100, learning_rate=1e-2, do_initialization=do_initialization, use_combined_res=True, do_MAP=do_MAP, do_HMC=do_HMC, 
        ...
        NegLog_combined = np.inf
        try:
            NegLog_empirical = train(x, Y, N_opt=2000, N_hmc=100, learning_rate=1e-2, do_initialization=do_initialization, use_empirical_res=True, do_MAP=do_MAP, do_HMC=do_HMC,
                hyper_pars=hyper_pars, save_dir=save_dir, folder_name=folder_name, subfolder_name=subfolder_name, folder_name_separable=folder_name_separable, verbose=True)
        except:
