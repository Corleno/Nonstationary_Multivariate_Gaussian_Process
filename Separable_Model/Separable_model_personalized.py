...
def separable_model(x, Y, trend, scale, x_scale, n_opt=1000, n_hmc=100):
    ...
    if do_pos_analysis:
        ...
        tilde_l_hist, tilde_sigma_hist, L_vec_hist, tilde_sigma2_err_hist = posterior_analysis.vec2pars(sample, N,
                                                                                                        M)  # array
        posterior_analysis.visualization_pos(x.numpy(), tilde_l_hist, tilde_sigma_hist, save_dir=save_dir, folder_name=folder_name, subfolder_name=subfolder_name)
...
def load_data(rank):
    ...
    x = origx / x_scale
    return x, Y, trend, scale, x_scale, attributes
...
if __name__ == "__main__":
    ...
    x, Y, trend, scale, x_scale, attributes = load_data(ID2index[ID])

    ts = time.time()
    separable_model(x, Y, trend, scale, x_scale, n_opt=1000)
